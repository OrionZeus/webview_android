...
var SendRedEnv = function (_widget_1$Widget) {
    ...
    _createClass(SendRedEnv, [{
        ...
        value: function sendRedEnv() {
            ...
            var url = '';
            var title = '';
            var lanSet = store_1.find('languageSet');
            var lan = lanSet.languageList[lanSet.selected];
            if (this.props.rtype === 0) {
                // tslint:disable-next-line:max-line-length
                url = pull_1.sharePerUrl + "?type=" + interface_1.RedEnvelopeType.Normal + "&rid=" + this.props.rid + "&lm=" + window.encodeURIComponent(this.props.message) + "&lan=" + lan;
                title = this.state.cfgData.redEnvType[0];
            } else if (this.props.rtype === 1) {
                // tslint:disable-next-line:max-line-length
                url = pull_1.sharePerUrl + "?type=" + interface_1.RedEnvelopeType.Random + "&rid=" + this.props.rid + "&lm=" + window.encodeURIComponent(this.props.message) + "&lan=" + lan;
                title = this.state.cfgData.redEnvType[1];
            } else {
                url = pull_1.sharePerUrl + "?cid=" + this.props.rid + "&type=" + interface_1.RedEnvelopeType.Invite + "&lan=" + lan;
