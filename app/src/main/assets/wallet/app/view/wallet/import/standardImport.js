    ...
    _createClass(StandardImport, [{
        ...
        value: function nextClick(e) {
            if (this.state.mnemonic.length <= 0) {
                root_1.popNew('app-components1-message-message', { content: this.state.cfgData.tips });
                return;
            }
            if (!genmnemonic_1.isValidMnemonic(constants_1.lang, this.state.mnemonic)) {
                root_1.popNew('app-components1-message-message', { content: this.state.cfgData.invalidMnemonicTips });
                return;
            }
