    ...
    _createClass(ExportPrivateKey, [{
        ...
        value: function exportPrivateKeyETH(addrs) {
            ...
            for (var j = 0; j < addrs.length; j++) {
                ...
                var addr = addrs[j];
                var balance = tools_1.getAddrInfoByAddr(addr, 'ETH').balance;
                keys.push({ addr: addr, balance: balance, privateKey: privateKey });
            }
            return keys;
        }
        // 导出BTC私钥
        ...
        value: function exportPrivateKeyBTC(addrs) {
            ...
            for (var j = 0; j < addrs.length; j++) {
                ...
                var addr = addrs[j];
                var balance = tools_1.getAddrInfoByAddr(addr, 'BTC').balance;
                keys.push({ addr: addr, balance: balance, privateKey: privateKey });
            }
            wlt.lock();
            return keys;
        ...
        value: function exportPrivateKeyERC20Token(addrs, currencyName) {
            ...
            for (var j = 0; j < addrs.length; j++) {
                ...
                var privateKey = wlt.exportPrivateKey();
                var addr = addrs[j];
                var balance = tools_1.getAddrInfoByAddr(addr, currencyName).balance;
                keys.push({ addr: addr, balance: balance, privateKey: privateKey });
            }
            return keys;
        }
