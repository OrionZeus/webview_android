...
class AudioRecorder : BaseJSModule() {
    ...
    fun start(callBack:(callType: Int, prames: Array<Any>)->Unit) {
        ...
        if (stream != null) {
            callBack(JSCallback.FAIL, arrayOf("AudioRecord isnt stop!"))
            return
        }
        ...
        PermissionsUtil.requestPermission(ctx!!, object : PermissionListener {
            ...
            override fun permissionDenied(permission: Array<String>) {
                callBack(JSCallback.FAIL, arrayOf("user denied permission"))
    ...
    private fun startImpl() {
        try {
            ...
            this.callBack(JSCallback.SUCCESS, arrayOf(""))
            ...
            callBack(JSCallback.FAIL, arrayOf("start Audio Record failed"))
            //JSCallback.callJS(activity, webView, callbackId, JSCallback.FAIL, "start Audio Record failed")
        }

    }

    @Synchronized fun stop(callBack:(callType: Int, prames: Array<Any>)->Unit) {
        this.callBack = callBack
        if (stream == null) {
            callBack(JSCallback.FAIL, arrayOf("AudioRecord isnt start!"))
    ...
    @Synchronized fun drop(callBack:(callType: Int, prames: Array<Any>)->Unit) {
        if (stream == null) {
            callBack(JSCallback.FAIL, arrayOf("AudioRecord isnt start!"))
            //JSCallback.callJS(activity, webView, callbackId, JSCallback.FAIL, "AudioRecord isnt start!")
            return
    ...
    private fun pcmToMp3(pcm: ShortArray) {
        ...
        if (size <= 0) {
            this.callBack(JSCallback.FAIL, arrayOf("Mp3 encode failed"))
    ...
    @Synchronized private fun handleRecordData() {
    ...
    companion object {
        ...
        private external fun lameMp3Close()

        private external fun lameMp3Encode(buffer_l: ShortArray, buffer_r: ShortArray?, samples: Int, mp3buf: ByteArray): Int

        private external fun lameMp3Flush(mp3buf: ByteArray): Int
    }
}
