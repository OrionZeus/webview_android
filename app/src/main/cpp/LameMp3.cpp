...
void Java_com_kuplay_kuplay_module_LameMp3_close(JNIEnv *env, jclass type){
    lame_close(glf);
    glf = NULL;
}

jint Java_com_kuplay_kuplay_module_LameMp3_encode(JNIEnv *env, jclass type, jshortArray buffer_l_,
...
jint Java_com_kuplay_kuplay_module_LameMp3_flush(JNIEnv *env, jclass type, jbyteArray mp3buf_) {
    jbyte *mp3buf = env->GetByteArrayElements(mp3buf_, NULL);
...
void Java_com_kuplay_kuplay_module_LameMp3_init__IIIII(JNIEnv *env, jclass type, jint inSampleRate, jint outChannel,
